...
| 6.7b | ✅ done (StreakCounter only) | 2026-05-06 | 2026-05-06 | StreakCounter component CRIADO em Sidebar footer (acima PlanCountdown) — lê `streaks.current_streak` de `/analytics` (sem novo backend signal). Bump motion (scale 1.15 spring overshoot 600ms) quando streak aumenta. Tooltip recorde pessoal. Esconde quando collapsed OU current=0. **StreakLostScreen não implementado** — precisaria backend signal `streak.lost_pending` (gap real, não bloqueio cosmético). |
...
| 7.3 | ✅ done | 2026-05-06 | 2026-05-06 | PricingDrawer F-NEW-8: migration `010_create_user_pricing_overrides.sql` (UNIQUE user_id+model_key), service `pricingOverrideService` (list/upsert/delete/getEffectivePricing), tokenService patch usa override async no webhook insert (não retroativo), routes settings extend (GET/PUT/DELETE /api/settings/pricing), useCustomPricing hook react-query, PricingDrawer right-anchored side panel (model picker + 4 rates inputs + list overrides ativos + Salvar/Remover), SettingsPage CTA "Customizar pricing" no header. Patrick precisa restartar backend pra picar endpoints (tsx watcher hot-reload pode não pegar service novo). |
